...
public class SjcAuthRegisterModel {
    ...
    public String register(SjcRegisterDto dto) {
        ...
        Long counts = sjcUserRoleRepository.selectCount(Wrappers.lambdaQuery(SjcUserRoleEntity.class)
                .eq(SjcUserRoleEntity::getSysUserId, user.getId())
                .eq(SjcUserRoleEntity::getRoleId, sjcRole.getId()));
        if (counts == 0L || counts == null) {
            ...
            sjcUserRole.setIsDelete(0);
            sjcUserRole.setCreateTime(LocalDateTime.now());
            sjcUserRole.setUpdateTime(LocalDateTime.now());
